...
class SBrick
	...
	# array of 4 values (an array of values that will be passed to SBrickQDCommand.port_command_hex_string)
	def quick_drive(port_values)
		...
		@last_qd_command = port_values.map {|x| SBrickQDCommand.port_command_hex_string(x)}.join('')
